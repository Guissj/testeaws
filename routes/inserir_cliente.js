...
router.put('/', function(req, res) {
    ...
    connection2.query("insert into logs(log)Values('"+data.cpfCnpj+"')",function (err,rows,fields){
        ...
        connection2.query("insert into DadosClientes(cpfCnpj,nome,email,cep,endereco,bairro,uf,profissao,telefoneContato,dataNascimento)Values('" + data.cpfCnpj + "','" + data.nome + "','" + data.email + "','" + data.cep + "','" + data.endereco + "','" + data.bairro + "','" + data.uf + "','" + data.profissao + "','" + data.telefoneContato + "','" + data.dataNascimento + "')", function (err, rows, fieds) {
            console.log(err);
            if(err==null){
